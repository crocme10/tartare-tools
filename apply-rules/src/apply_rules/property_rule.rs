        ...
        .filter(|((object_type, object_id, property_name), property)| {
            ...
            if !(is_valid_property() || is_physical_mode_property_for_line() || is_trip_property_for_line() || is_equipment_property_for_stop_point()) {
                report.add_warning(
                    ...
                    ReportCategory::UnknownPropertyName,
                );
                return false;
            }
...
fn property_old_value_do_not_match(report: &mut Report<ReportCategory>, p: &PropertyRule) {
    report.add_warning(
        format!(
        ...
        ReportCategory::OldPropertyValueDoesNotMatch,
    )
}

fn property_unknown_value(report: &mut Report<ReportCategory>, p: &PropertyRule, val: &str) {
    report.add_warning(
        format!(
        ...
        if let Ok(i) = T::from_str(&p.property_value) {
            *field = Some(i);
            ...
            report.add_warning(
                ...
                ReportCategory::NonConvertibleString,
            );
        }
    } else {
        property_old_value_do_not_match(report, p);
...
fn wkt_to_geo(
    ...
    if let Ok(wkt) = wkt::Wkt::from_str(wkt) {
        ...
        report.add_warning(
            ...
            ReportCategory::GeometryNotValid,
        );
        None
    }
}
