...
struct NetexidfRoute(Route);
impl TryFrom<&Element> for NetexidfRoute {
    type Error = failure::Error;
    fn try_from(route_element: &Element) -> Result<NetexidfRoute> {
        ...
        Ok(NetexidfRoute(route))
    }
}

impl Deref for NetexidfRoute {
    type Target = Route;
...
pub fn get_stop_point(
    ...
    let stop_point_idx = if let Some(sp_idx) = stop_points.get_idx(&stop_point_id) {
        ...
        let vst = virtual_stop_point.to_owned();
        stop_points.push(StopPoint {
            id: vst.id,
            name: vst.name,
            code: vst.code,
            codes: vst.codes,
            object_properties: vst.object_properties,
            comment_links: vst.comment_links,
            visible: vst.visible,
            coord: vst.coord,
            stop_area_id: vst.stop_area_id,
            timezone: vst.timezone,
            geometry_id: vst.geometry_id,
            equipment_id: vst.equipment_id,
            fare_zone_id: vst.fare_zone_id,
            level_id: vst.level_id,
            platform_code: vst.platform_code,
            stop_type: vst.stop_type,
        })?
    };
    Ok(stop_point_idx)
}
    ...
    I: Iterator<Item = &'a Element>,
{
    ...
    for route_element in route_elements {
        let route = skip_error_and_log!(NetexidfRoute::try_from(route_element), LogLevel::Warn);
        ...
        let idf_route = route.to_owned();
        routes.push(Route {
            id: idf_route.id,
            line_id: idf_route.line_id,
            name: idf_route.name,
            direction_type: idf_route.direction_type,
            codes: idf_route.codes,
            ..Default::default()
        })?;
    }
    Ok(routes)
...
mod tests {
    ...
    mod parse_vehicle_journeys {
        use super::*;
        use crate::netexidf::modes::IDFMMode;
        use pretty_assertions::assert_eq;
        use transit_model::objects::{Comment, CommentType, Dataset};
    ...
    mod get_stop_point {
        ...
        fn stop_points() -> (
            ...
            .unwrap();
            let virtual_stop_points = CollectionWithId::new(vec![VirtualStopPoint {
                id: String::from("vsp0"),
                ..Default::default()
            }])
            .unwrap();
