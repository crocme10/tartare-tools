...
impl From<StopArea> for VirtualStopPoint {
...
fn load_stop_areas<'a>(
    ...
    for stop_place in stop_places.iter().filter(|sp| !has_parent_site_ref(sp)) {
        ...
        if loaded_stop_area.id.contains("monomodalStopPlace:") {
            virtual_stop_points.push(VirtualStopPoint::from(loaded_stop_area.clone()))?;
        }
    ...
    for stop_place in stop_places.iter().filter(|sp| has_parent_site_ref(sp)) {
        ...
        let loaded_stop_area = load_stop_area(stop_place, proj)?;
        let mut virtual_stop_point = VirtualStopPoint::from(loaded_stop_area.clone());
