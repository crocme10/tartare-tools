...
fn test_map_no_force() {
    test_in_tmp_dir(|path| {
        ...
        let input_dir = "./fixtures/map-object-with-osm/input";
        let model = ntfs::read(input_dir).unwrap();
        let enriched_model = improve_stop_positions::enrich_object_codes(
            Path::new("./fixtures/map-object-with-osm/marseille-lite.osm.pbf"),
            model,
            ntfs_network_to_osm,
            false,
        )
        ...
        compare_output_dir_with_expected(
            ...
            "./fixtures/map-object-with-osm/output/no_force",
        );
    });
}
...
fn test_map_force() {
    test_in_tmp_dir(|path| {
        ...
        compare_output_dir_with_expected(
            &path,
            Some(vec!["object_codes.txt"]),
            "./fixtures/map-object-with-osm/output/force",
        );
    });
}
